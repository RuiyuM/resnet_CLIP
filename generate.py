...
def generate_command(sampling_method, dataset_name, gpu_id):
    ...
    for method in sampling_methods:
        for dataset_name in datasets:
            for kc in datasets[dataset_name]['known_class']:
                command = generate_command(method, dataset_name, gpu_counter % num_gpus)
                f.write(command[0] + "\n")
                gpu_counter += 1
