...
def get_labeled_features(args, models, labeled_in_loader):
    ...
    features_in = torch.tensor([]).cuda().cuda()
...
def mqnet_train_epoch(args, models, optimizers, criterion, delta_loader, meta_input_dict):
    ...
    while (batch_idx < args.steps_per_epoch):
        for data in delta_loader:
            ...
            #inputs, labels, indexs = data[0].cuda(), data[1].cuda(), data[2].cuda()
...
def meta_train(args, models, optimizers, schedulers, criterion, labeled_in_loader, unlabeled_loader, delta_loader):
    ...
    for i, idx in enumerate(indices):
        meta_input_dict[idx.item()] = [meta_input[i].cuda(), in_ood_masks[i]]
...
def train_epoch_LL(args, models, epoch, criterion, optimizers, dataloaders):
    ...
    for data in dataloaders:
        ...
        inputs, labels = data[1][0].cuda(), data[1][1].cuda()
...
def train(args, models, criterion, optimizers, schedulers, dataloaders):
    ...
    if args.query_strategy in ['Random', 'Uncertainty', 'Coreset', 'BADGE', 'CCAL', 'SIMILAR']:
    ...
    elif args.query_strategy in ['LL']: #MQNet
        for epoch in tqdm(range(args.epochs), leave=False, total=args.epochs):
            train_epoch_LL(args, models, epoch, criterion, optimizers, dataloaders)
            schedulers['backbone'].step()
            schedulers['module'].step()

    elif args.query_strategy in ['MQNet']: #MQNet
...
def get_models(args, nets, model, models, dataset):
    # Normal
    if args.query_strategy in ['Random', 'Uncertainty', 'Coreset', 'BADGE']:
    ...
    # SIMILAR
    elif args.query_strategy =='SIMILAR':
    ...
    # MQNet
        ...
        loss_module = nets.__dict__['LossNet'](args.im_size).cuda()

        model_ = model + '_CSI'
        model_csi = nets.__dict__[model_](args.channel, args.num_IN_class, args.im_size).cuda()


        #backbone = nets.nets_utils.MyDataParallel(backbone).cuda()
        #loss_module = nets.nets_utils.MyDataParallel(loss_module).cuda()
        #model_csi = nets.nets_utils.MyDataParallel(model_csi).cuda()
...
def get_optim_configurations(args, models):
    ...
    # Normal
    if args.query_strategy in ['Random', 'Uncertainty', 'Coreset', 'BADGE', 'SIMILAR']:
