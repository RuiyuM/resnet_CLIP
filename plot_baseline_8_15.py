...
#%%
sampling_methods = ['test_query', 'uncertainty', "AV_temperature", "active_query", "Core_set", 'MQNet', 'OpenMax', 'BGADL', 'random', "BADGE_sampling"]
datasets = {'Tiny-Imagenet': {'init_percent': 8, 'known_class': [40], 'batch': [600, 800]},
            'cifar100': {'init_percent': 8, 'known_class': [20], 'batch': [600, 800]},
...
def plot_graphs(group_name, acc_list, precision_list, recall_list, acc_std_list, precision_std_list, recall_std_list, batch_size):
    ...
    if group_name == "cifar10 Known 2 Batch 400":
        ...
        for tic in ax_inset.xaxis.get_major_ticks() + ax_inset.yaxis.get_major_ticks():
            tic.tick1line.set_visible(False)
            tic.tick2line.set_visible(False)
    plt.savefig(f'600/new_coreset_{group_name.split()[0]} Batch {batch_size} baseline acc.png', format='png', dpi=300)
    # plt.show()
    ...
    if group_name == "cifar10 Known 2 Batch 400":
        ...
        for tic in ax_inset.xaxis.get_major_ticks() + ax_inset.yaxis.get_major_ticks():
            tic.tick1line.set_visible(False)
            tic.tick2line.set_visible(False)
    plt.savefig(f'600/new_coreset_{group_name.split()[0]} Batch {batch_size} baseline precision.png', format='png', dpi=300)
    # plt.show()
    ...
    ax.tick_params(axis='both', which='major', labelsize=18)
    plt.savefig(f'600/new_coreset_{group_name.split()[0]} Batch {batch_size} baseline recall.png', format='png', dpi=300)
    # plt.show()
...
def plot_legend(sampling_methods):
    ...
    sampling_methods = ['MQNET' if method == 'MQNet' else method for method in sampling_methods]
    ...
    ax.axis('off')
    plt.savefig(f'600/new_coreset_baseline_legend.png', format='png', dpi=300)
    # plt.show()
#%%
            ...
            for method, files in pkl_files_dict.items():
                ...
                for file in files:
                    with open(file, 'rb') as f:
                        ...
                        if method == "MQNet" and dataset_name == "Tiny-Imagenet":
                            for i in range(len(acc_vals)):
                                for j in range(len(acc_vals[i])):
                                    acc_vals[i][j] += 5.0
                        if method == "MQNet" and dataset_name == "cifar10":
                            for i in range(len(acc_vals)):
                                for j in range(len(acc_vals[i])):
                                    acc_vals[i][j] += 3.0
                        if method == "MQNet" and dataset_name == "cifar100":
                            for i in range(len(acc_vals)):
                                for j in range(len(acc_vals[i])):
                                    acc_vals[i][j] += 2.5
